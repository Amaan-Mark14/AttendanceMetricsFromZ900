...
class EmployeeManager(QWidget):
    ...
    def setup_ui(self):
        ...
        self.employees_table.setHorizontalHeaderLabels([
            "En No", "Name", "Start Time", "End Time", "Grace (min)", "Min Hours", "Status", "Actions"
        ])
        ...
        try:
            ...
            query = '''
                SELECT e.en_no, e.name, s.start_time, s.end_time,
                       s.grace_period, s.minimum_hours, e.is_active
                FROM employees e
                LEFT JOIN shifts s ON e.shift_id = s.id
                ORDER BY CAST(e.en_no AS INTEGER)
            '''
            ...
            for row_idx, employee in enumerate(employees):
                ...
                configure_btn.clicked.connect(lambda checked, en_no=employee[0]: self.configure_employee(en_no))
                self.employees_table.setCellWidget(row_idx, 7, configure_btn)
        ...
        try:
            ...
            cursor.execute('''
                SELECT e.en_no, e.name, e.shift_id, s.start_time, s.end_time,
                       s.grace_period, s.minimum_hours, s.maximum_hours
                FROM employees e
                LEFT JOIN shifts s ON e.shift_id = s.id
                WHERE e.en_no = ?
            ''', (en_no,))
...
class EmployeeConfigDialog(QDialog):
    ...
    def save_configuration(self):
        try:
            ...
            # Update employee with new shift
            cursor.execute('''
                UPDATE employees SET shift_id = ? WHERE en_no = ?
            ''', (shift_id, self.en_no))
